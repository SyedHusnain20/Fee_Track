...
CategoryFeeDefault rows, and (as of Step 9) attendance timing settings.
...
Enrollment depends on CategoryFeeDefault the same way. As of Step 9, the
kiosk also needs per-category start_time/grace_minutes SystemSetting rows
to compute on-time/late — seeded here with PLACEHOLDER values (not your
school's real schedule; review and correct them on the /settings page).
...
DEFAULT_CATEGORY_TIMING = {
    FeeCategory.SCHOOL: ("08:00", 15),
    FeeCategory.COACHING: ("16:00", 15),
    FeeCategory.ENGLISH: ("17:00", 15),
    FeeCategory.COMPUTER: ("18:00", 15),
}
...
def seed_attendance_settings(session: Session) -> None:
    ...
    for category, (start_time, grace_minutes) in DEFAULT_CATEGORY_TIMING.items():
        start_key = f"{category.value}_start_time"
        grace_key = f"{category.value}_grace_minutes"
        if start_key not in existing_keys:
            session.add(SystemSetting(key=start_key, value=start_time))
            added += 1
        if grace_key not in existing_keys:
            session.add(SystemSetting(key=grace_key, value=str(grace_minutes)))
            added += 1
